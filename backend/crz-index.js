...
app.post('/api/upload/file', async (req, res) => {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      req.once('aborted', () =>
        reject(new Error('Browser upload was interrupted'))
      );
    });
...
const server = app.listen(config.port, () =>
  console.log(`CRZ web/API server listening on :${config.port}`)
);
server.requestTimeout = 0;

registerMirrorBotCommands()
  .catch(error =>
    console.error('Could not register CRZ Bot commands:', error.message)
  );

mirrorBot.launch()
  .then(() => console.log('CRZ Bot started'))
  .catch(error => {
    console.error('CRZ Bot failed to start:', error);
    process.exitCode = 1;
  });

process.once('SIGINT', () => mirrorBot.stop('SIGINT'));
process.once('SIGTERM', () => mirrorBot.stop('SIGTERM'));
